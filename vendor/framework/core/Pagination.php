...
class Pagination
{
    ...
    public function get()
    {
        ...
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            ...
            if ($page == $this->current_page) {
                $links .= '<li class="page-item active"><a class="page-link" href="javascript:void(0);">' . $page . '</a></li>';
            } else {
    ...
    private function generateHtml($page, $text = null)
    {
        ...
        $currentURI_query = parse_url($currentURI) ;
        $currentURI_query = isset($currentURI_query['query']) ? array($currentURI_query['query']) : false;

        $pageUrl = $this->index . $page;
        
        preg_match('~\/(?<!\?)\?(?!\?)page=[0-9]+~', $currentURI, $result);

        //Если в url есть код языка
        $curr_url = ltrim($currentURI, '/') ;

        if(array_key_exists($curr_url , $langs)){
        	$pageUrl = $this->index . $page;
        }
        
        if ($currentURI_query) { 
            preg_match('~(?<!\?)\?(?!\?)page=[0-9]+~', $currentURI, $result);
            parse_str($currentURI_query[0], $res);
            if (empty($result)) {  
                $pageUrl = str_replace('?', '&', $pageUrl); 
            }
            if (count($currentURI_query)  && count($res) > 1) { 
               $pageUrl = str_replace('?', '&', $pageUrl); 
           }
        }
        ...
        return '<li class="page-item"><a class="page-link" href="' . $currentURI . $pageUrl . '">' . $text . '</a></li>';
    }
